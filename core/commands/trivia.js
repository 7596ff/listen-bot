module.exports = (message, client, helper) => {
    ...
    if (command == "points") {
        client.pg.query("SELECT * FROM scores ORDER BY score DESC;").then(res => {
            let found_obj = client.core.util.search_members(message.channel.guild.members, split_content.slice(1));
            let found = found_obj.found ? found_obj.all[0] : message.author.id;
