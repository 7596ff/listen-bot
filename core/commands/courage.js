...
async function courage(message, client, helper) {
    let get_buffer = client.core.util.get_buffer;
    let od_heroes = client.core.json.od_heroes;
    let all_items = client.core.json.items;
    ...
    Promise.all(to_download.map(uri => get_buffer(`http://cdn.dota2.com/apps/dota2${uri}`, `${__dirname}/../..${uri}`))).then(res => { // rofl
